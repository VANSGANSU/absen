...
function formatDate(date: Date, options: Intl.DateTimeFormatOptions) {
  return new Intl.DateTimeFormat("en-US", { ...options, timeZone: "Asia/Jakarta" }).format(date)
}
...
function formatShortRange(start: Date, end: Date) {
  return `${formatDate(start, { month: "short", day: "2-digit" })} - ${formatDate(end, { month: "short", day: "2-digit", year: "numeric" })}`
}
...
function isInRange(record: AttendanceRecordEntry, selectedRange: SharedDashboardRange, now: Date): boolean {
  ...
  switch (selectedRange) {
    ...
    case "This month":
      return recordDate >= getStartOfMonth(today) && recordDate <= getEndOfMonth(today)
    case "This year":
      return recordDate >= getStartOfYear(today.getFullYear()) && recordDate <= getEndOfYear(today.getFullYear())
    case "Last year":
      return recordDate >= getStartOfYear(today.getFullYear() - 1) && recordDate <= getEndOfYear(today.getFullYear() - 1)
    case "Last 7 days": {
...
export function AttendanceListOverview() {
  ...
  const [selectedGroup,  setSelectedGroup]  = React.useState<(typeof groupOptions)[number]>("All Groups")
  ...
  React.useEffect(() => {
    const handlePointerDown = (event: MouseEvent) => {
      ...
      if (!statusRef.current?.contains(target)) setIsStatusOpen(false)
  ...
  React.useEffect(() => { setCurrentPage(1) }, [selectedRange, selectedStatus, selectedGroup, searchQuery])
  ...
  // ── Toggle Entry button: buka inline form ATAU navigasi ke /add ───────────────
  const handleEntryClick = () => {
    if (showAddForm) {
      // Tutup form inline yang sedang terbuka
      setShowAddForm(false)
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center">
            ...
            <div className="flex flex-wrap gap-3">
                          ...
                          {items.map((item) => {
                            ...
                            return (
                              <button
                                key={item.label}
                                type="button"
                                onClick={() => { setSelectedRange(item.label); setIsDateOpen(false) }}
                    ...
                    {groupOptions.map((option) => {
                      ...
                      return (
                        <button
                          key={option}
                          type="button"
                          onClick={() => { setSelectedGroup(option); setIsGroupOpen(false) }}
                          className={`flex w-full items-center justify-between rounded-[0.7rem] px-4 py-3 text-left text-[1.05rem] ${
                            active ? "bg-slate-100 text-slate-950" : "text-slate-900 hover:bg-slate-50"
                          }`}
                        >
              ...
              {/* Entry — toggle inline form (menggantikan tombol emerald) */}
              <button
                type="button"
                onClick={handleEntryClick}
                className={`inline-flex items-center gap-3 rounded-[0.95rem] px-5 py-3 text-[1.05rem] font-medium transition ${
                  showAddForm
                  ...
                  <tbody>
                      ...
                      <tr className="border-t border-slate-300 bg-slate-50">
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            className="w-full rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500"
                          />
                        </td>
                        <td className="px-4 py-3">
                          <input
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500"
                          />
                        </td>
                        <td className="px-4 py-3">
                          <input type="time" value={newForm.checkIn} onChange={(e) => setNewForm((f) => ({ ...f, checkIn: e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500" />
                        </td>
                        <td className="px-4 py-3">
                          <input type="time" value={newForm.breakIn} onChange={(e) => setNewForm((f) => ({ ...f, breakIn: e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500" />
                        </td>
                        <td className="px-4 py-3">
                          <input type="time" value={newForm.breakOut} onChange={(e) => setNewForm((f) => ({ ...f, breakOut: e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500" />
                        </td>
                        <td className="px-4 py-3">
                          <input type="time" value={newForm.checkOut} onChange={(e) => setNewForm((f) => ({ ...f, checkOut: e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500" />
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="inline-flex h-9 items-center gap-1.5 rounded-[0.6rem] bg-black px-3 text-sm font-medium text-white transition hover:bg-slate-800 disabled:opacity-50"
                            >
                    ...
                    {paginatedRecords.length ? (
                      paginatedRecords.map((record) => {
                        ...
                        if (isEditing) {
                          return (
                            <tr key={record.id} className="border-t border-blue-200 bg-blue-50">
                              ...
                              <td className="px-4 py-3">
                                <input type="text" value={editForm.memberName} onChange={(e) => setEditForm((f) => ({ ...f, memberName: e.target.value }))} className="w-full rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                              </td>
                              <td className="px-4 py-3">
                                <input type="text" value={editForm.memberDepartment} onChange={(e) => setEditForm((f) => ({ ...f, memberDepartment: e.target.value }))} className="w-full rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                              </td>
                              <td className="px-4 py-3">
                                <input type="date" value={editForm.date} onChange={(e) => setEditForm((f) => ({ ...f, date: e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                              </td>
                              <td className="px-4 py-3"><input type="time" value={editForm.checkIn}  onChange={(e) => setEditForm((f) => ({ ...f, checkIn:  e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" /></td>
                              <td className="px-4 py-3"><input type="time" value={editForm.breakIn}  onChange={(e) => setEditForm((f) => ({ ...f, breakIn:  e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" /></td>
                              <td className="px-4 py-3"><input type="time" value={editForm.breakOut} onChange={(e) => setEditForm((f) => ({ ...f, breakOut: e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" /></td>
                              <td className="px-4 py-3"><input type="time" value={editForm.checkOut} onChange={(e) => setEditForm((f) => ({ ...f, checkOut: e.target.value }))} className="rounded-[0.6rem] border border-slate-300 px-2 py-1.5 text-[0.95rem] text-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" /></td>
                              <td className="px-4 py-3 text-sm italic text-slate-400">auto</td>
                              <td className="px-4 py-3 text-sm italic text-slate-400">auto</td>
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  <button type="button" onClick={() => handleEditSave(record.id)} className="inline-flex h-9 items-center gap-1.5 rounded-[0.6rem] bg-blue-600 px-3 text-sm font-medium text-white transition hover:bg-blue-700">
                                    <Save className="size-3.5" />
                                    Simpan
                                  </button>
                                  <button type="button" onClick={handleEditCancel} className="inline-flex h-9 w-9 items-center justify-center rounded-[0.6rem] border border-slate-200 text-slate-500 transition hover:bg-slate-100">
                                    <X className="size-4" />
                                  </button>
                                </div>
                              </td>
                        ...
                        return (
                          <tr key={record.id} className="border-t border-slate-200 transition hover:bg-slate-50">
                            ...
                            <td className="px-4 py-5 text-[1.05rem] text-slate-500">{record.memberDepartment}</td>
                            <td className="px-4 py-5 text-[1.05rem] text-slate-500">
                              {new Intl.DateTimeFormat("en-US", { month: "short", day: "2-digit", year: "numeric", timeZone: "Asia/Jakarta" }).format(new Date(`${record.date}T12:00:00+07:00`))}
                            </td>
                            ...
                            <td className="px-4 py-5">
                              <span className={`rounded-full px-3 py-1 text-sm font-medium ${statusInfo.style}`}>
                                {statusInfo.label}
                              </span>
                            </td>
                            <td className="px-4 py-5">
                              <div className="flex items-center gap-1">
                                <button type="button" onClick={() => handleStartEdit(record)} className="inline-flex h-9 w-9 items-center justify-center rounded-full text-slate-400 transition hover:bg-blue-50 hover:text-blue-500" aria-label="Edit record">
                                  <Pencil className="size-4" />
                                </button>
                                <button type="button" onClick={() => handleDeleteRecord(record.id)} className="inline-flex h-9 w-9 items-center justify-center rounded-full text-slate-400 transition hover:bg-red-50 hover:text-red-500" aria-label="Delete record">
                      ...
                      <tr>
                        <td colSpan={11} className="h-64 px-4 py-8 text-center">
                          {records.length === 0 ? (
                            <div className="flex flex-col items-center gap-3">
                              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-100">
                                <CalendarDays className="size-7 text-slate-400" />
                              </div>
                              <p className="text-[1.15rem] font-medium text-slate-700">Belum ada data yang tercatat</p>
                              <p className="text-[1rem] text-slate-400">Klik tombol &ldquo;Entry&rdquo; untuk mulai mencatat absensi</p>
                            </div>
                          ) : (
                            <p className="text-[1.1rem] italic text-slate-500">Tidak ada data yang cocok dengan filter saat ini.</p>
                          )}
                        </td>
                      </tr>
                    )}
                ...
                .map((page) => (
                  <button
                    ...
                    className={`inline-flex h-11 min-w-11 items-center justify-center rounded-[0.85rem] px-4 text-sm font-medium ${
                      page === safePage ? "bg-black text-white" : "border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                    }`}
                  >
                    {page}
                  </button>
