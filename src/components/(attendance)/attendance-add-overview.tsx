...
type MemberItem = OrganizationMember
...
function isBreakInValid(timeStr: string) {
  if (!timeStr) return true
  const [h, m] = timeStr.split(":").map(Number)
  const mins = h * 60 + m
  return mins >= 13 * 60 && mins <= 13 * 60 + 45
}
  ...
  return (
    <div className="rounded-[0.9rem] border border-slate-200 bg-white p-2.5">
      ...
      <input
        ...
        className={`mt-1.5 w-full rounded-[0.5rem] border ${error ? 'border-red-300 bg-red-50 text-red-900' : 'border-slate-200'} px-1.5 py-1 text-center text-[0.9rem] text-slate-950 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500`}
      />
      {error && <p className="mt-1 text-[0.65rem] text-red-500 text-center leading-tight">{error}</p>}
    </div>
  )
}
...
export function AttendanceAddOverview({ initialDate }: AttendanceAddOverviewProps) {
  ...
  const [selectedGroup, setSelectedGroup] = React.useState<string>("All Groups")
  ...
  const [existingRecords, setExistingRecords] = React.useState<AttendanceRecordEntry[]>([])
  ...
  React.useEffect(() => {
    const handleDown = (e: MouseEvent) => {
      ...
      if (!groupRef.current?.contains(t)) setIsGroupOpen(false)
    ...
    const load = async () => {
      try {
        ...
        if (isMounted) {
          ...
          setExistingRecords(nextRecords)
        }
      } catch (error) {
        if (isMounted) {
          setSubmitError(error instanceof Error ? error.message : "Failed to load attendance data.")
        }
      }
    }
  ...
  const canPerform = React.useCallback(
    (field: keyof TimeFields): boolean => {
      ...
      const isBreakTime = currentMinutes >= 13 * 60 && currentMinutes <= 13 * 60 + 45
  ...
  const filteredMembers = members.filter((m) => {
    ...
    const matchesGroup =
      entryMode === "single" || selectedGroup === "All Groups" ? true : m.group === selectedGroup
  ...
  const handleSingleSubmit = () => {
    void (async () => {
      ...
      if (memberHasRecordOnDate(selectedMember.id)) {
        setSubmitError("This member already has an attendance record for this date.")
        setAlert({ type: "error", message: "This member already has an attendance record for this date." })
      ...
      try {
        await insertAttendanceRecords(supabase, [newRecord])
        ...
        setAlert({ type: "error", message: error?.message || "Failed to save attendance." })
        setTimeout(() => setAlert(null), 3000)
      } finally {
  ...
  const handleBatchSubmit = () => {
    void (async () => {
      ...
      if (validMembers.length === 0) {
        setSubmitError("All selected members already have attendance records for this date.")
        setAlert({ type: "error", message: "All selected members already have attendance records for this date." })
        setTimeout(() => setAlert(null), 3000)
        return
      }
      setIsSubmitting(true)
      ...
      try {
        await insertAttendanceRecords(supabase, newRecords)
        setSubmitSuccess(true)
        setAlert({ type: "success", message: `${validMembers.length} attendance record(s) added successfully.` })
        setTimeout(() => {
          setAlert(null)
          router.push("/dashboard/attendance/list")
          router.refresh()
        ...
        console.error("Attendance batch submission error:", error)
        setSubmitError(error?.message || "Failed to save attendance.")
        setAlert({ type: "error", message: error?.message || "Failed to save attendance." })
        setTimeout(() => setAlert(null), 3000)
      } finally {
  ...
  const CalendarDropdown = () => (
    <div ref={calendarRef} className="relative">
      ...
      {isCalendarOpen && (
        <div className="absolute left-0 top-[calc(100%+0.5rem)] z-30 w-[17.5rem] overflow-hidden rounded-[1rem] border border-slate-200 bg-white shadow-xl">
                ...
                {entryMode === "batch" && (
                  <div className="space-y-3">
                    <div ref={groupRef} className="relative">
                      ...
                      {isGroupOpen && (
                        <div className="absolute left-0 top-[calc(100%+0.5rem)] z-30 w-full overflow-hidden rounded-[1rem] border border-slate-200 bg-white p-2 shadow-xl">
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="text-slate-500">
                              {(() => {
                                ...
                                if (mins < 13 * 60) return "Break starts at 13:00"
                                if (mins <= 13 * 60 + 45) return "Break available now"
                        ...
                        <div className="flex items-center justify-between gap-4 rounded-[1rem] bg-slate-50 px-5 py-4">
                          <div>
                            ...
                            <p className="text-[0.9rem] uppercase tracking-[0.08em] text-slate-400">
                              {selectedMember.group} · {formatInputDate(selectedDate)}
                            </p>
                          </div>
                          <button
                            type="button"
                            onClick={handleSingleSubmit}
                            disabled={isSubmitting || !timeFields.checkIn || memberHasRecordOnDate(selectedMember.id) || !isBreakInValid(timeFields.breakIn)}
                ...
                <div className="space-y-4">
                              ...
                              <div className="mt-3 grid grid-cols-2 gap-2">
                                {timeInputConfigs.map((item) => (
                                  <RetroTimeCard
                                    ...
                                    error={item.field === "breakIn" && times.breakIn && !isBreakInValid(times.breakIn) ? "Break only 13:00-13:45" : undefined}
                                  />
                                ))}
                              </div>
                            )}
                  ...
                  <div className="flex items-center justify-between gap-4 rounded-[1rem] bg-slate-50 px-5 py-4">
                    ...
                    <button
                      ...
                      disabled={isSubmitting || addedBatchMembers.some(id => !isBreakInValid(getBatchMemberTime(id).breakIn))}
                      className="inline-flex items-center gap-2 rounded-[0.95rem] bg-black px-6 py-3 text-[1.05rem] font-medium text-white transition disabled:opacity-60"
                    >
                      <Save className="size-5" />
          ...
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              {timeInputConfigs.map((item) => (
                <div key={item.field} className="space-y-1.5">
                  <label className="flex items-center justify-between text-sm font-semibold uppercase tracking-wide text-slate-600">
                    <div className="flex items-center gap-1.5">
                      <item.icon className="size-4" />
                      {item.label}
                    </div>
                    {item.field === "breakIn" && timeFields.breakIn && !isBreakInValid(timeFields.breakIn) && (
                      <span className="text-[0.7rem] text-red-500 normal-case tracking-normal">
                        Only available 13:00 - 13:45
                      </span>
                    )}
                  </label>
                  <input
                    ...
                    className={`w-full rounded-lg border ${item.field === "breakIn" && timeFields.breakIn && !isBreakInValid(timeFields.breakIn) ? 'border-red-300 bg-red-50' : 'border-slate-200'} px-3 py-2.5 text-center text-base focus:border-blue-500 focus:ring-1 focus:ring-blue-500`}
                  />
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={!timeFields.checkIn || isSubmitting || !isBreakInValid(timeFields.breakIn)}
