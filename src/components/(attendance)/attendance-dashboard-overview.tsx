...
function getDateRangeBounds(selectedRange: SharedDashboardRange, currentTime: Date): { start: Date; end: Date } {
...
export function AttendanceDashboardOverview({
  ...
  const loadData = React.useCallback(async (retryCount = 0) => {
    ...
    try {
      ...
      const [mResult, gResult, aResult] = settled
  ...
  const rangeMeta = React.useMemo(() => {
    const meta = getRangeMeta(selectedRange, currentTime)
    return meta
  }, [selectedRange, currentTime])

  const rangeAttendance = React.useMemo(() => {
    const { start, end } = getDateRangeBounds(selectedRange, currentTime)
    return attendance.filter((r) => {
      const d = parseAttendanceDate(r.date)
      return d >= start && d <= end
    })
  }, [attendance, selectedRange, currentTime])

  const activeMembers = React.useMemo(() => members.filter(m => m.status === 'Active'), [members])

  const organizationCardsData = React.useMemo(() => {
    return [
      {
        label: "Total Members",
      ...
      {
        label: "Avg Group Size",
        value: groups.length > 0 ? (activeMembers.length / groups.length).toFixed(1) : "0",
        note: "Members per Groups",
        icon: Target,
        iconTone: "text-emerald-400",
      },
    ]
  }, [activeMembers, groups])

  const kpiCardsData = React.useMemo(() => {
    let presentCount = 0
    let lateCount = 0
    let absentCount = 0
    ...
    rangeAttendance.forEach(record => {
      ...
      if (status.includes('present') && !status.includes('late')) {
        presentCount++
        const hours = record.workHours ? parseFloat(record.workHours) : 0
        if (!isNaN(hours)) totalWorkHours += Math.max(0, hours)
    ...
    const totalRecords = rangeAttendance.length
    const attendanceRate = totalRecords > 0 ? Math.max(0, (presentCount / totalRecords) * 100) : 0
    const punctualityRate = presentCount > 0 ? Math.max(0, ((presentCount - lateCount) / presentCount) * 100) : 0
    const absenteeismRate = totalRecords > 0 ? Math.max(0, (absentCount / totalRecords) * 100) : 0
    const avgWorkHours = presentCount > 0 ? Math.max(0, totalWorkHours / presentCount) : 0

    return [
      {
        label: "Attendance Rate",
        value: `${attendanceRate.toFixed(1)}%`,
        note: "Based on overall records",
        icon: CheckCircle2,
        border: "border-l-[5px] border-l-emerald-500",
        tint: "from-emerald-50 to-white",
        bar: "bg-emerald-200",
  ...
  return (
    <div className="space-y-6">
      <header className="animate-rise relative z-20 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between pb-2">
        <div className="flex-1">
          <h1 className="text-3xl font-bold tracking-tight text-slate-8 00 sm:text-[3rem]">
            Dashboard
          </h1>
          <div className="mt-2 flex flex-wrap items-center gap-3 text-sm font-medium text-slate-500">
            <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-slate-950">
        ...
        <div ref={dropdownRef} className="relative z-30 w-full sm:w-auto">
          <button
            ...
            className="flex w-full items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-slate-700 shadow-sm transition-all hover:bg-slate-50 sm:min-w-72"
          >
            <div className="flex items-center gap-3">
              <CalendarDays className="size-5 text-slate-400" />
              <span className="text-base font-semibold">{rangeMeta.pickerLabel}</span>
            </div>
            <ChevronDown className={`size-4 transition-transform duration-200 ${isMenuOpen ? 'rotate-180' : ''}`} />
          </button>

          {isMenuOpen && (
            <div className="absolute right-0 top-[calc(100%+0.5rem)] z-40 w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl sm:w-80">
              {Object.entries(groupedOptions).map(([group, items], index) => (
                <div key={group} className={index ? "border-t border-slate-200" : ""}>
                  <div className="flex items-center gap-2 px-4 py-3 text-sm font-semibold text-slate-400 uppercase tracking-wider bg-slate-50/50">
                    ...
                    {items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isSelected ? <CheckCircle2 className="size-4 text-emerald-600" /> : null}
                        </button>
                      )
                    })}
                  </div>
      ...
      <section className="space-y-5">
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          {organizationCardsData.map((card) => (
            <article
              key={card.label}
          ...
          <article className="rounded-[1.6rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid min-h-[26rem] items-end">
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  { label: "Present", color: "bg-emerald-500", count: rangeAttendance.filter(r => r.status?.toLowerCase() === "present").length },
                  { label: "Late", color: "bg-amber-500", count: rangeAttendance.filter(r => r.status?.toLowerCase() === "late").length },
                  { label: "Absent", color: "bg-red-500", count: rangeAttendance.filter(r => r.status?.toLowerCase().includes("absent")).length },
                  { label: "Leave / Excused", color: "bg-blue-500", count: rangeAttendance.filter(r => { const s = r.status?.toLowerCase() ?? ""; return s.includes("leave") || s.includes("excused") }).length },
                ].map((item) => (
      ...
      <section className="space-y-5">
        ...
        <article className="rounded-[1.6rem] border border-slate-200 bg-white p-6 shadow-sm">
          <h3 className="text-[1.1rem] font-semibold text-slate-950">
            Top 5 Groups by Attendance Rate
          </h3>
          <p className="mt-2 text-[1.05rem] text-slate-500">
